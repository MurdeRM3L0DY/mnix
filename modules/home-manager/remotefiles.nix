  ...
  fileType = baseDir: {...}: {
    options = {
      url = lib.mkOption {
        ...
        '';
        example = "https://github.com/MurdeRM3L0DY/dotfiles.git";
      };

      path = lib.mkOption {
        type = lib.types.str;
        description = ''
          The path of the remote file. By default, it will be relative to the
          home directory.
        '';
...
in {
  options.home.remotefiles = lib.mkOption {
    type = with lib.types; attrsOf (submodule (fileType config.home.homeDirectory));
    description = ''
      An attribute set of remote files and directories to be declaratively put
    ...
    example = lib.literalExpression ''
      {
        "library/dotfiles" = {
          url = "https://github.com/MurdeRM3L0DY/dotfiles.git";
          type = "git";
        };
  ...
  config = lib.mkIf (cfg != {}) {
    lib.remotefiles.symlink = name: p: {
      source =
        config.lib.file.mkOutOfStoreSymlink
        "${config.home.remotefiles.${name}.path}/${toString p}";
    };

    home.activation = {
      fetchRemoteFiles = let
        remoteFilesCmds =
          lib.mapAttrsToList
          (path: value: let
      ...
      in
        lib.hm.dag.entryAfter ["writeBoundary"] ''
          $DRY_RUN_CMD ${pkgs.bash}/bin/bash -c ${lib.getExe script}
        '';
    };
  };
}
